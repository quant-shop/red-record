...
Despite Goodwin's analysis, research on contemporary efforts to understand the role of data in making sense of interactions with police at a systems level. This bring us to the Washington Post fatal force data. To frame this data, we discuss the literature on the history of race, police brutality, violence and social control. We begin with historical data on lynching, as provided by Ida B. Wells.

## Show some data from Ida B. Wells (table)
Connect to social control
...
fatal_black_men_2023 <- fatal_black_men_2023 %>%
  mutate(state_lower = tolower(state)) 

fatal_black_men_2023 <- fatal %>% 
  filter(
    race_category == "Black" | race == "B",
...
# write a function to extract 2020 - 2023 (given covid)

fatal_black_women_2022 <- fatal %>% 
  filter(
    race_category == "Black" | race == "B",
...
```{r}
# while the wells data does not contain information on Black women, we include 
fatal_black_women_2022
...
# Tables (move up to after goodwin - intro)
